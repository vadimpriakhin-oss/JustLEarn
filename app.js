...
function loadTerms() {
    ...
    try {
        const parsed = stored ? JSON.parse(stored) : DEFAULT_TERMS;
        if (Array.isArray(parsed)) {
            appState.userTerms = parsed;
        } else {
            appState.userTerms = [...DEFAULT_TERMS];
            localStorage.removeItem('justlearnTerms');
        }
    } catch (e) {
        console.error('Failed to load terms from localStorage:', e);
        appState.userTerms = [...DEFAULT_TERMS];
        localStorage.removeItem('justlearnTerms');
    }
}
...
function saveTerms() {
    try {
        ...
        console.error('Failed to save terms to localStorage:', e);
    }
}
...
function renderTermFields() {
    ...
    appState.userTerms.forEach((item, index) => {
        ...
        input.addEventListener('change', (e) => {
            ...
            if (appState.userTerms[idx] !== undefined) {
                appState.userTerms[idx].term = e.target.value;
            }
        });
        ...
        textarea.addEventListener('change', (e) => {
            const idx = parseInt(e.target.dataset.index, 10);
            if (appState.userTerms[idx] !== undefined) {
                appState.userTerms[idx].definition = e.target.value;
            }
        });
...
function removeTermField(index) {
    if (index >= 0 && index < appState.userTerms.length) {
...
function saveAndHome() {
    appState.userTerms = appState.userTerms.filter(t => isValidTerm(t));
    if (appState.userTerms.length === 0) {
...
function startQuiz(mode) {
    if (!appState.userTerms || appState.userTerms.length === 0) {
...
function showQuizScreen() {
    if (!appState.shuffledTerms || appState.shuffledTerms.length === 0) {
        showHomeScreen();
        return;
    }
    if (!appState.currentQuizMode) {
        showHomeScreen();
        return;
    }
    ...
    if (appState.currentQuizMode === 'true-false') {
        ...
        mcOptions = shuffle([term.definition, ...wrong]);
        content += '<div style="display: flex; flex-direction: column; gap: 10px;">';
        mcOptions.forEach((opt, i) => {
            content += `<button data-answer-index="${i}" style="background: #1a1a2e; color: #00ffcc; border: 2px solid #00ffcc; padding: 15px; border-radius: 8px; cursor: pointer; text-align: left; font-weight: bold;">${String.fromCharCode(65 + i)}: ${escapeHtml(opt)}</button>`;
        });
        content += '</div>';
    ...
    if (appState.currentQuizMode === 'multiple-choice') {
        document.querySelectorAll('[data-answer-index]').forEach((btn) => {
            ...
            btn.addEventListener('click', () => {
                checkAnswerMultiple(mcOptions[idx]);
            });
        });
    }
}
...
function checkAnswer(userAnswer) {
    ...
    const correctIsTrue = correct.isTrue !== false;
    const isCorrect = (userAnswer === 'true') === correctIsTrue;
    if (isCorrect) {
